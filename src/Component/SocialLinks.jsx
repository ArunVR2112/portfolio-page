            ...
            <ul>
                {links.map(({ id, link, nameofTag, icon, download }) => (
                    <li key={id} className='flex justify-center items-center h-10 w-40 px-4 ml-[-100px] hover:rounded-md duration-300 hover:ml-[-10px] bg-gray-500'>
                        <a href={process.env.PUBLIC_URL + link} download={download} target='_blank' rel="noopener noreferrer" className='flex justify-between items-center w-full text-white rounded-md'>
                            <>
                                {nameofTag}{icon}
                            </>
                        </a>
